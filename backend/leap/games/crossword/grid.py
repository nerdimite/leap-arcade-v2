...
def entry_cells(start_row: int, start_col: int, direction: str, length: int) -> List[Tuple[int, int]]:
...
def walk_entry(grid: List[List[Optional[str]]], start_row: int, start_col: int, direction: str, length: int) -> str:
...
def validate_seeded_entry(grid: List[List[Optional[str]]], answer: str, start_row: int, start_col: int, direction: str) -> bool:
...
def validate_grid_consistency(grid: List[List[Optional[str]]], entries: List[Dict[str, Any]]) -> None:
    """Validate that:
    ...
    for entry in entries:
        ...
        if not validate_seeded_entry(grid, ans, r, c, d):
            raise ValueError(f"Entry {ans!r} in direction {d} at ({r},{c}) does not match the grid or hits a blocked cell.")
        ...
        for c in range(cols):
            if grid[r][c] is not None:
                if (r, c) not in covered_cells:
                    raise ValueError(f"Grid cell ({r}, {c}) is non-null but not covered by any entry.")
            else:
                if (r, c) in covered_cells:
                    raise ValueError(f"Grid cell ({r}, {c}) is null but covered by an entry.")


def build_skeleton(grid: List[List[Optional[str]]], entries: List[Dict[str, Any]]) -> Tuple[List[List[Optional[Dict[str, Any]]]], List[Dict[str, Any]]]:
