...
describe("postWikiPlay", () => {
  it("POST /api/games/wiki/play returns parsed WikiPlayResponse", async () => {
    server.use(
      http.post(
        ({ request }) =>
          new URL(request.url).pathname.endsWith("/api/games/wiki/play"),
        async ({ request }) => {
...
describe("postWikiNavigate", () => {
  it("POST navigate with title JSON body", async () => {
    server.use(
      http.post(
        ({ request }) =>
          new URL(request.url).pathname.endsWith("/api/games/wiki/navigate"),
        async ({ request }) => {
    ...
    const res = await postWikiNavigate("Next", {
      baseUrl: "http://localhost:3000",
      cookieHeader: "y=1",
    })
...
describe("postWikiTimeout", () => {
  it("POST /api/games/wiki/timeout returns WikiPlayResponse", async () => {
    server.use(
      http.post(
        ({ request }) =>
          new URL(request.url).pathname.endsWith("/api/games/wiki/timeout"),
