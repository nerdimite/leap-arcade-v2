...
describe("postPlay", () => {
  it("sends cookie and Authorization-free JSON POST; returns parsed PlayResponse", async () => {
    server.use(
      http.post(
        ({ request }) => new URL(request.url).pathname.endsWith("/api/games/rapid-fire/play"),
        async ({ request }) => {
...
describe("postAnswer", () => {
  it("sends question_id, selected_option, time_ms JSON body", async () => {
    server.use(
      http.post(
        ({ request }) => new URL(request.url).pathname.endsWith("/api/games/rapid-fire/answer"),
        async ({ request }) => {
...
describe("postAbandon", () => {
  it("returns parsed AbandonResponse; handler invoked once per call", async () => {
    ...
    server.use(
      http.post(
        ({ request }) => new URL(request.url).pathname.endsWith("/api/games/rapid-fire/abandon"),
        () => {
          calls += 1;
          return HttpResponse.json({
            result: {
    ...
    const res = await postAbandon({ baseUrl: "http://localhost:3000", cookieHeader: "token=abc" });
    expect(calls).toBe(1);
