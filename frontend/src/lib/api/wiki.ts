import type { WikiNavigateResponse, WikiPlayResponse } from "@/services/wiki/schema";
import { WikiNavigateResponseSchema, WikiPlayResponseSchema } from "@/services/wiki/schema";
...
export async function postWikiPlay(options: PostWikiPlayOptions = {}): Promise<WikiPlayResponse> {
...
export async function postWikiTimeout(options: PostWikiPlayOptions = {}): Promise<WikiPlayResponse> {
  const origin = resolveOrigin(options.baseUrl);
  const headers: Record<string, string> = {
    accept: "application/json",
  };
