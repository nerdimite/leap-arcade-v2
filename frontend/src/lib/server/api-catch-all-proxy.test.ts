...
describe("forwardApiToBackend", () => {
  ...
  it("forwards POST with body intact", async () => {
    ...
    const req = new NextRequest("http://localhost:3000/api/games/rapid-fire/play", {
      method: "POST",
      headers: {
        cookie: "token=tok",
  ...
  it("strips the browser-only /api prefix for nested FastAPI paths", async () => {
    ...
    const req = new NextRequest("http://localhost:3000/api/players/me/sessions", {
      method: "GET",
  ...
  it("returns 401 when token cookie is absent (non-login path)", async () => {
    ...
    const req = new NextRequest("http://localhost:3000/api/leaderboard", { method: "GET" });
