...
function buildUpstreamUrl(backendOrigin: string, pathSegments: string[]): URL {
  ...
  return new URL(`/${path}`, backendOrigin.endsWith("/") ? backendOrigin : `${backendOrigin}/`);
}

function buildUpstreamHeaders(request: NextRequest, bearerToken: string | undefined): Headers {
