  ...
  const body = (
    <>
      ...
      <div
        ...
        style={locked ? { filter: "saturate(0.45)" } : { boxShadow: "0 0 18px var(--accent)" }}
      />
      ...
      <div className="flex gap-4 p-[18px]">
        ...
        <div className="min-w-0">
          <h3 className="font-pixel text-[11px] leading-[1.5] text-ink">{name}</h3>
          <p className="mt-2.5 text-[13px] leading-[1.55] text-ink-dim">{description}</p>
        </div>
      </div>
      ...
      <div className="mt-0.5 flex items-center justify-end px-[18px] pb-[18px]">
        {status.kind === "play" ? (
          <span
            className="rounded-full bg-[var(--accent)] px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.5px] text-bg shadow-[var(--shadow-cabinet-sm)]"
          >
            {status.label}
          </span>
        ) : (
      ...
      {locked ? (
        <Lock
          className="absolute right-3.5 top-3.5 size-3.5 text-ink-faint"
          aria-hidden
        />
      ) : null}
    </>
