...
export function ResultCard(props: {
  ...
  const stats = [
    ...
    {
      label: "Time",
      value: `${props.timeTakenSeconds.toFixed(1)}s`,
      tone: "text-ink",
    },
  ]
      ...
      <div className="p-6">
        ...
        <dl className="mt-6 grid grid-cols-2 gap-3">
          {stats.map((s) => (
            ...
            >
              <dd className={`font-pixel text-[14px] tabular-nums ${s.tone}`}>
                {s.value}
              </dd>
              <dt className="mt-1.5 text-[10px] font-bold tracking-[1px] text-ink-faint uppercase">
                {s.label}
              </dt>
            </div>
          ))}
