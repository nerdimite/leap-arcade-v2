  ...
  switch (action.type) {
    ...
    case "TYPE_LETTER": {
      ...
      if (state.context.lockedCells.has(startKey)) {
        ...
        if (
          !advanced ||
          (advanced.row === cursor.row && advanced.col === cursor.col)
        ) {
          return state
        }
        cursor = advanced
    ...
    case "CHECK_HIT": {
      ...
      return {
        ...
        solveFlashEntryIds: state.solveFlashEntryIds.includes(
          action.payload.entryId
        )
    ...
    case "CHECK_MISS":
      return {
        ...state,
        missFlashEntryIds: state.missFlashEntryIds.includes(
          action.payload.entryId
        )
