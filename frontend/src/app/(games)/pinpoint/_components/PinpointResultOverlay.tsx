...
const CONFETTI = [
  { dx: "-58px", dy: "-34px", rot: "-160deg", color: "var(--four)", delay: "0ms" },
  { dx: "-32px", dy: "-50px", rot: "120deg", color: "var(--pin)", delay: "20ms" },
  { dx: "-10px", dy: "-58px", rot: "-90deg", color: "var(--wiki)", delay: "0ms" },
  { dx: "14px", dy: "-56px", rot: "200deg", color: "var(--rapid)", delay: "40ms" },
  { dx: "36px", dy: "-48px", rot: "-140deg", color: "var(--four)", delay: "10ms" },
  { dx: "58px", dy: "-30px", rot: "90deg", color: "var(--pin)", delay: "30ms" },
  { dx: "-46px", dy: "-14px", rot: "160deg", color: "var(--wiki)", delay: "50ms" },
...
function useCountUp(target: number, durationMs = 420): number {
  const [value, setValue] = useState(target <= 0 || prefersReducedMotion() ? target : 0);
  ...
  return (
    ...
    >
      <div className="relative overflow-visible rounded-[var(--radius)] border-2 border-four bg-panel px-7 py-5 text-center shadow-[var(--shadow-cabinet)] motion-safe:animate-rf-verdict-in">
        <span className="pointer-events-none absolute left-1/2 top-3 motion-reduce:hidden">
          {CONFETTI.map((p) => (
            <span
              key={`${p.dx}-${p.dy}-${p.rot}`}
        ...
        <p className="mt-2.5 text-[10px] font-bold uppercase tracking-[1px] text-ink-faint tabular-nums">
          {timeBonus > 0 ? `${baseScore} base + ${timeBonus} time` : `${baseScore} base`}
        </p>
      </div>
    </div>
  );
...
export function PinpointResultOverlay(props: PinpointResultOverlayProps) {
  ...
  return <SolvedOverlay baseScore={baseScore} timeBonus={timeBonus} cluesUsed={cluesUsed} />;
}
