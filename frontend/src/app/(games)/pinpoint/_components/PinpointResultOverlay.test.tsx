...
describe("PinpointResultOverlay", () => {
  ...
  it("reveals the total score with the base and time bonus breakdown", () => {
    render(<PinpointResultOverlay kind="solved" baseScore={400} timeBonus={66} cluesUsed={3} />);
  ...
  it("omits the time line when there is no bonus", () => {
    render(<PinpointResultOverlay kind="solved" baseScore={400} timeBonus={0} cluesUsed={4} />);
    const status = screen.getByRole("status");
  ...
  it("praises a one-clue solve louder than a last-ditch one", () => {
    render(<PinpointResultOverlay kind="solved" baseScore={500} timeBonus={0} cluesUsed={1} />);
    expect(screen.getByRole("status")).toHaveTextContent("PINPOINTED");
    cleanup();
    render(<PinpointResultOverlay kind="solved" baseScore={100} timeBonus={0} cluesUsed={5} />);
