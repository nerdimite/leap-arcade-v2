  ...
  return (
    <ol className="flex flex-col gap-2.5" aria-label="Clue badges">
      {Array.from({ length: PINPOINT_CLUE_COUNT }, (_, index) => {
        ...
        return (
          <li
            // biome-ignore lint/suspicious/noArrayIndexKey: fixed-length positional clue slots that never reorder
            key={index}
            ...
            className={cn(
              ...
              isRevealed
                ? "animate-pinpoint-badge-reveal border-[var(--accent)]/45 bg-[var(--accent)]/12"
                : "border-line bg-bg-2",
              isShaking &&
                "animate-pinpoint-badge-shake border-cross bg-cross/20"
            )}
          >
            <span
              className={cn(
