import { PINPOINT_TIME_BONUS_DECAY_MS } from "@/lib/constants";

export function elapsedMsFromStartedAt(startedAt: string, nowMs: number = Date.now()): number {
...
/** Visual decay cue: neutral → warm → urgent → stable no-bonus past 90s. */
export function stopwatchToneClass(elapsedMs: number): string {
  if (elapsedMs >= PINPOINT_TIME_BONUS_DECAY_MS) {
