...
function HudStat({
  ...
  return (
    ...
    >
      <span className="text-[9px] uppercase tracking-[1px] text-ink-faint">{label}</span>
      <span
        className={cn(
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 z-30 -mx-3 backdrop-blur supports-[backdrop-filter]:bg-bg/85 sm:-mx-5 lg:-mx-7">
        <header className="flex flex-wrap items-center justify-between gap-x-6 gap-y-3 border-b-2 border-line bg-bg/95 px-3 py-3.5 sm:px-5 lg:px-7">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-pixel text-[8px] uppercase tracking-[2px] text-[var(--accent)]">
          ...
          <div className="flex flex-wrap items-stretch gap-2.5">
            ...
            <HudStat label="Rounds" value={`${completedCount}/${current.puzzle_count}`} />
            <HudStat label="Time left" value={formatMs(displayMs)} tone="time" low={lowTime} />
        ...
        <div className="flex items-center gap-4 border-b-2 border-line bg-bg/85 px-3 py-3 sm:px-5 lg:px-7">
          ...
          <span className="whitespace-nowrap font-pixel text-[11px] text-ink-dim tabular-nums">
            PUZZLE{" "}
      ...
      <div className="mt-6 flex items-start gap-5 rounded-[var(--radius)] border-2 border-line bg-panel px-5 py-4 shadow-[var(--shadow-cabinet-sm)]">
        ...
        <p className="max-w-[64ch] text-[16px] font-medium italic leading-snug text-ink">
          {current.clue}
        </p>
      </div>
