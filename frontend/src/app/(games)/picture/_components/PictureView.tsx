...
export function PictureView(props: PictureViewProps) {
  const { viewState, onAnswerChange, onSubmit, onSkip, onInputAnimationEnd, onSessionExpired } =
    props;

  if (viewState.status === "result") {
    return (
      <div style={PIC_ACCENT}>
        <ResultScreen result={viewState.result} onBackToLobby={props.onBackToLobby} />
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto flex max-w-xl flex-col gap-4 p-6" style={PIC_ACCENT}>
      <GameHeader
        gameId="picture"
      ...
      <form className="flex flex-col gap-3" onSubmit={onSubmit}>
        ...
        <input
          ...
          disabled={isPending}
        />
        {wrongMessage ? <p className="text-[14px] text-cross">{wrongMessage}</p> : null}
