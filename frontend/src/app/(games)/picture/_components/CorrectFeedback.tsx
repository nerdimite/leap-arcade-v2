...
const CONFETTI = [
  { dx: "-58px", dy: "-40px", rot: "-160deg", color: "var(--pic)", delay: "0ms" },
  { dx: "-30px", dy: "-56px", rot: "120deg", color: "var(--four)", delay: "20ms" },
  { dx: "-8px", dy: "-62px", rot: "-90deg", color: "var(--pic)", delay: "0ms" },
  { dx: "16px", dy: "-58px", rot: "200deg", color: "var(--rapid)", delay: "40ms" },
  { dx: "36px", dy: "-48px", rot: "-140deg", color: "var(--four)", delay: "10ms" },
  { dx: "58px", dy: "-34px", rot: "90deg", color: "var(--pic)", delay: "30ms" },
  { dx: "-46px", dy: "-16px", rot: "160deg", color: "var(--four)", delay: "50ms" },
  { dx: "48px", dy: "-12px", rot: "-110deg", color: "var(--rapid)", delay: "60ms" },
];

/** Game-show host verdicts: rotate on solo wins, escalate as the streak builds. */
const SOLO_PHRASES = ["NICE", "GOT IT", "SHARP EYE", "CLEAN", "SPOT ON"] as const;
...
export function CorrectBurst({ scoreDelta, streak, token }: CorrectBurstProps) {
  return (
    <div className="pointer-events-none absolute inset-0 z-10 overflow-visible">
      ...
      <span aria-hidden className="absolute left-1/2 top-1/2 motion-reduce:hidden">
        {CONFETTI.map((p) => (
          <span
            key={`${p.dx}-${p.dy}-${p.rot}`}
      ...
      <span
        aria-hidden
        className="absolute left-1/2 top-1/2 motion-safe:animate-picture-correct-stamp motion-reduce:animate-picture-correct-stamp-soft"
      >
...
export function ScorePop({ scoreDelta }: ScorePopProps) {
  ...
  return (
    <span
      aria-hidden
      className="pointer-events-none absolute -top-3 right-1 font-pixel text-[12px] tabular-nums text-four opacity-0 motion-safe:animate-picture-score-pop motion-reduce:animate-picture-score-pop-soft"
    >
      +{scoreDelta}
    </span>
  );
