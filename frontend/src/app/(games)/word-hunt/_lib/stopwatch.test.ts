...
describe("stopwatchToneClass", () => {
  it("stays neutral early in the puzzle", () => {
    expect(stopwatchToneClass(5_000)).toContain("muted")
  })

  it("warms as elapsed crosses the halfway mark of the decay window", () => {
    expect(stopwatchToneClass(300_000)).toContain("amber")
  })

  it("intensifies near the bonus floor", () => {
    expect(stopwatchToneClass(480_000)).toContain("destructive")
  })

  it("settles into no-bonus state past decay", () => {
    expect(stopwatchToneClass(WORD_HUNT_TIME_DECAY_MS)).toContain("no-bonus")
    expect(stopwatchToneClass(700_000)).toContain("no-bonus")
  })
})
