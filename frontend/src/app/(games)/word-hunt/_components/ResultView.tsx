  ...
  return (
    <div className="mx-auto flex max-w-lg flex-col gap-6 p-6 pb-10">
      <header className="space-y-1">
        <h1 className="text-xl font-semibold tracking-tight">Word Hunt</h1>
      ...
      {result.found_words.length > 0 ? (
        <section className="space-y-3">
          <h2 className="text-sm font-semibold tracking-tight">Words found</h2>
          <ul className="overflow-hidden rounded-xl border">
            {result.found_words.map((found) => (
              ...
              >
                <span className="font-medium">{found.word}</span>
                <span className="text-sm text-muted-foreground">{found.clue}</span>
              </li>
            ))}
          </ul>
        </section>
