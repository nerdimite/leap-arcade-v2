...
function isCellInTrace(
  row: number,
  col: number,
  trace: Coordinates | null | undefined
): boolean {
  if (!trace) {
    return false
  }
  return cellsInTrace(trace).some(
    (cell) => cell.row === row && cell.col === col
  )
}

/** Position of a cell along a trace (start = 0), or -1 if it isn't on it. Drives the per-letter stagger. */
function traceIndexOf(
  row: number,
  col: number,
  trace: Coordinates | null | undefined
): number {
  ...
  return cellsInTrace(trace).findIndex(
    (cell) => cell.row === row && cell.col === col
  )
}
...
export function LetterGrid({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: resolveCellFromPoint only reads the stable gridRef, so re-subscribing window listeners each render is unnecessary
  useEffect(() => {
  ...
  return (
    <div
      ref={gridRef}
      className="inline-grid touch-none gap-1 select-none"
      style={{ gridTemplateColumns: `repeat(${cols}, minmax(2rem, 1fr))` }}
    >
      {Array.from({ length: rows }, (_, row) =>
        Array.from({ length: cols }, (_, col) => {
          const letter = grid[row]?.[col] ?? ""
          const found = highlighted.some((trace) =>
            isCellInTrace(row, col, trace)
          )
          ...
          return (
            <button
              ...
              style={
                landing ? { animationDelay: `${landIndex * 45}ms` } : undefined
              }
              disabled={disabled}
