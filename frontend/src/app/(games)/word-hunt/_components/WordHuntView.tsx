...
import type { Coordinates, PuzzleState, Result } from "@/services/word_hunt/schema";
...
export function WordHuntView(props: WordHuntViewProps) {
  const { viewState, onDragStart, onDragMove, onDragEnd, onSubmit, onBackToLobby } = props;
  ...
  const { puzzle, sessionScore, highlights, dragPreview, missFlash, landAnimation, showScoreIncrement, disabled } =
    viewState;

  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-6 p-6 lg:flex-row" style={WORD_ACCENT}>
      <div>
        <GameHeader
          gameId="word_hunt"
      ...
      <div className="flex-1">
        <h2 className="mb-3 text-[10px] font-bold uppercase tracking-[1px] text-ink-faint">Clues</h2>
        <ClueListPanel clues={puzzle.clues} />
        <button
          type="button"
          className="mt-6 inline-flex h-11 items-center justify-center rounded-[var(--radius)] border-2 border-[var(--accent)] bg-[var(--accent)] px-5 text-[12px] font-extrabold uppercase tracking-[1.5px] text-bg shadow-[var(--shadow-cabinet-sm)] transition-[transform,box-shadow] duration-150 ease-[var(--ease-arcade)] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none disabled:pointer-events-none disabled:opacity-50 motion-reduce:transition-none"
          disabled={disabled}
          onClick={onSubmit}
        >
          Submit
