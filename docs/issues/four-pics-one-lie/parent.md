...
- `POST /games/four-pics/play` returns the next unattempted question for new and mid-game players (idempotent for a given active attempt), and the result block for completed players
- `POST /games/four-pics/answer` validates `selected_index` (0–3), records the attempt with server-clamped `time_ms`, computes `score` (base + time bonus on correct, 0 on wrong), and returns the next question inline (or the result block on the final question)
- `POST /games/four-pics/abandon` ends the session as `abandoned`, closes any active attempt as `wrong` with score 0, and surfaces unattempted questions as `not_reached` in the result
- Per-question scoring is `100 + max(0, ⌊50 × (1 − ms / 30_000)⌋)` on correct, `0` on wrong; final session score is the sum of per-question scores
- All seeded questions are dynamically shuffled per session — no shuffle order is stored; the service picks uniformly at random from unattempted questions on each `play` advance
- `odd_one_out_index` is never included in any API response (defensive: enforced by API schema, not relying on serializer omission)
- The per-question stopwatch is server-authoritative — `started_at` is set when the question is first served and never reset; refreshes resume from the server timestamp
...
- Frontend `(games)/four-pics/page.tsx` no longer renders the placeholder; player can play through the full game with a polished overlay UX, live stopwatch, and result screen
- All e2e API journey and lifecycle tests pass
